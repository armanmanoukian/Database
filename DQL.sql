...
-- Query 3 top-5 performing sellers bassed on total sales 
SELECT
    user_id,
    seller_company_name,
    total_sales
...
--- Query 4 top rated products with rating >= 4.0
SELECT
    p.product_id,
    p.product_name,
...
GROUP BY
    ...
    p.product_name
HAVING
    COUNT(r.review_id) >= 1        -- must have reviews
    AND AVG(r.review_rating) >= 4   
ORDER BY
    avg_rating DESC,
    reviews_count DESC;

--Query5 Product Price fluctuation
...
--Query 6 customer purtchase summary
SELECT
...
--Query 8 delivery process report
SELECT o.order_id, o.order_date, c.first_name || ' ' || c.last_name AS carrier_name, c.truck_number, w.warehouse_name, c.delivery_status, c.delivered_at, AVG(c.delivered_at - o.order_date) OVER() AS avg_delivery_days 
...
---Query 9 Refund Analysis Report
